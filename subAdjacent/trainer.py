...
def detect_anomalies(params, model, optimizer, val_loader):
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(val_loader, desc="Detecting anomalies")):
            if params.debug and i >= 50:
                break
            unscaled = unscale_features(batch['features'],"data/standard_scaler.pkl")
            ...
            # flatten
            rec_loss_flat = rec_loss.view(-1).cpu().numpy()
            sacon_flat = sacon_all_layers.view(-1).cpu().numpy()
    ...
    sacon_weights /= (sacon_weights.sum() + 1e-12)
    ...
    unscale_and_save_anomalies(
        ...
        threshold=threshold,
        feature_names=params.feature_columns,
        scaling_dir=".",
        output_csv=os.path.join(params.output_dir, "detected_anomalies.csv")
    )

    # Plot
    plot_loss_spans(all_rec, all_sacon, params.output_dir)
