...
def merge_insights_and_write_csv(db_path, table_name, row_insights, csv_output):
    ...
    Returns the percentage of rows that have insights.
    """
    conn = sqlite3.connect(db_path)
    ...
    total_rows = len(df)
    rows_with_insights = len(df[df["insight"] != ""])
    insight_percentage = (rows_with_insights / total_rows * 100) if total_rows > 0 else 0
    ...
    print(f"[INFO] Wrote {csv_output}")
    print(f"[INFO] Found insights in {rows_with_insights}/{total_rows} rows ({insight_percentage:.2f}%)")
    
    return insight_percentage
...
def main():
    ...
    for csv_file in tqdm(all_files, desc="Processing CSV files"):
        ...
        temp_output = os.path.join(output_folder, f"{base_name}_insights_temp.csv")
        insight_percentage = merge_insights_and_write_csv(db_path, TABLE_NAME, row_insights, temp_output)
        
        # 4) Rename the file to include the percentage
        final_output = os.path.join(output_folder, f"{base_name}_insights_{insight_percentage:.4f}pct.csv")
