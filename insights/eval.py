...
class AnomalyAnalyzer:
    ...
    def generate_visualizations(self, matched_df: pd.DataFrame, unmatched_df: pd.DataFrame, missed_df: pd.DataFrame) -> None:
    ...
    def _plot_file_rule_distribution(self, df: pd.DataFrame) -> None:
        ...
        plt.title('Distribution of Rules Across Files')
        plt.xlabel('File')
        plt.ylabel('Rule Type')
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        plt.savefig(self.config.plots_folder / 'file_rule_distribution.png')
        plt.close()

    def _plot_rule_analysis(self, df: pd.DataFrame) -> None:
        """Generate rule-based analysis plots"""
        plt.figure(figsize=(15, 10))
        ...
        ax1.set_title('Rule Occurrence and File Coverage')
        ...
        ax2.set_title('Average Occurrences per Affected File')
        ax2.set_xticklabels(stats_df['Rule'], rotation=45, ha='right')
        
        plt.tight_layout()
        plt.savefig(self.config.plots_folder / 'rule_analysis.png')
        plt.close()

    def _plot_model_performance_by_file(self, matched_df: pd.DataFrame, missed_df: pd.DataFrame) -> None:
        """Generate plots showing model performance per file"""
        plt.figure(figsize=(15, 10))
        ...
        for _, row in missed_df.iterrows():
            file = self.parse_filename_from_timestamp(row['timestamp_str'])
            if file:
        ...
        ax1.set_title('Model Detection Rate by File')
        ...
        ax2.set_title('Detected vs Missed Anomalies by File')
        ax2.set_xticklabels(ax2.get_xticklabels(), rotation=45, ha='right')
        
        plt.tight_layout()
        plt.savefig(self.config.plots_folder / 'model_performance_by_file.png')
        plt.close()

    def _plot_rule_correlation(self, df: pd.DataFrame) -> None:
        """Generate correlation analysis plots between rules"""
        # Create feature matrix for rules
        rule_features = pd.get_dummies(df['insight'].str.get_dummies(','))
        
        plt.figure(figsize=(12, 10))
        sns.heatmap(rule_features.corr(), annot=True, cmap='coolwarm', center=0)
        plt.title('Rule Correlation Matrix')
        plt.tight_layout()
        plt.savefig(self.config.plots_folder / 'rule_correlation.png')
        plt.close()
    ...
    def _plot_detection_performance(self, matched_df: pd.DataFrame, missed_df: pd.DataFrame) -> None:
        ...
        plt.title('Overall Model Detection Performance')
...
def main():
    ...
    analyzer.generate_visualizations(matched_df, unmatched_df, missed_df)
