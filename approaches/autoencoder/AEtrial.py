...
class TransformerAnomalyDetector(nn.Module):
    """
    This module tokenizes each row of categorical features by embedding each column,
    concatenates them, and projects to a common d_model. After adding positional encoding,
    the resulting sequence is passed through a Transformer encoder-decoder (autoencoder).
    
    In addition, a learnable cluster center is maintained. The training loss is a joint loss:
    (i) a reconstruction loss (cross–entropy per feature per token) and (ii) an adaptive cluster loss
    that is weighted per token by (1 – sigmoid(reconstruction error)).
    """
    ...
    def forward(self, src):
        ...
        tgt = src_emb
        decoded = self.transformer(src_emb, tgt)  # (seq_len, batch, d_model)
...
def compute_loss(model_output, target, model, lambda_cluster):
    ...
    The per–token reconstruction error is used to down–weight the cluster loss on “hard” (anomalous) tokens.
    """
    batch, seq_len, _ = target.size()
    device = target.device

    # Compute per-token reconstruction error for each feature (reduction='none')
    ...
    # Overall reconstruction loss (mean over all tokens)
    recon_loss = rec_error_tokens.mean()
    
    # Compute adaptive weight: when reconstruction error is high, weight is low.
    weight = 1.0 - torch.sigmoid(rec_error_tokens)
    
    latent_seq = model_output["reconstructed_sequence"]  # (B, T, d_model)
    # Expand the cluster center to match the shape of latent_seq.
    cluster_center = model.cluster_center.expand_as(latent_seq)
    
    # Unsqueeze weight to shape (B, T, 1) so it broadcasts properly.
...
def update_threshold(model, dataloader, params, device, k=2.0):
    ...
    The new threshold is set to: mean + k * std of the anomaly scores computed over the dataloader.
    """
    model.eval()
    n = 0
    mean = 0.0
    M2 = 0.0
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Updating threshold"):
...
def produce_anomalies(model, dataloader, params, device):
    """
    For each sequence in the dataloader, compute an anomaly score per time step based on:
      - Reconstruction error (summed cross entropy for each feature)
      - Cluster distance (Euclidean distance in latent space from the learned cluster_center)
    
    The final anomaly score is:
         anomaly_score = alpha * rec_error + beta * cluster_distance
    If the score exceeds params['threshold'], record the anomaly along with:
      - The timestamp
      - Both components of the anomaly score (reconstruction error, cluster distance)
      - The threshold used
      - For each feature, the original value and the predicted token.
    Returns:
        anomalies_df: DataFrame containing the detailed anomaly information.
        all_scores: List of numpy arrays with per–time–step anomaly scores.
    """
    model.eval()
    anomalies = []
    all_scores = []
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Producing anomalies"):
            ...
            # Get predicted tokens for each feature
            pred_sfn   = torch.argmax(output["predictions"]["SFN"], dim=-1)  # shape (B, T)
            ...
            for i in range(batch_size):
                # Ensure each sample’s timestamp list is as long as the sequence length.
...
class CSVValidationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Handle last sequence if it would exceed array bounds
...
def validate_csv(model, params, device):
    """
    Runs anomaly detection on the validation CSV with sequence-based processing
    """
    ...
    # Remove duplicate anomalies based on timestamp
...
# -------------------------- (Optional) Parquet Iterable Dataset --------------------------
...
def main():
    ...
    params = {
        ...
        'dim_feedforward': 256,
        'dropout': 0.1,
        'lambda_cluster': 2,   # weight for the cluster loss
        'alpha': 1.0,          # weight for reconstruction error in anomaly score
        'beta': 3.0,           # weight for latent cluster distance in anomaly score
        'threshold': 5,        # initial threshold (will be updated adaptively)
        'batch_size': 64,
        'learning_rate': 1e-3,
        'num_epochs': 1,
        # Paths for CSV validation (and ground truth)
    ...
    # Using the Parquet dataset for training:
    ...
    # (Optional) Learning rate scheduler, gradient clipping, and early stopping can be added here.
    
    best_f1 = 0.0
    best_model_path = os.path.join(params['output_dir'], 'best_model.pth')
    ...
    for epoch in range(1, params['num_epochs'] + 1):
        ...
        # Update threshold adaptively using the CSV validation dataset (or a subset of training data)
    ...
    # After training, run final anomaly detection on a test CSV and save anomalies.
    test_dataset = CSVValidationDataset(params['validation_csv_path'])
    test_loader = DataLoader(test_dataset, batch_size=params['batch_size'], shuffle=False, collate_fn=custom_collate_fn)
    anomalies_df, _ = produce_anomalies(model, test_loader, params, device)
    ...
    if not anomalies_df.empty:
        ...
        logging.warning("No anomalies detected to save.")
