...
def update_threshold(model, dataloader, params, device, k=2.0):
    ...
    M2 = 0.0
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Updating threshold", leave=False):
...
# -------------------------- Threshold Search Routine --------------------------
def search_best_threshold(model, params, device, candidate_thresholds):
    """
    Given a trained model and a list of candidate thresholds, evaluate the CSV validation set
    for each threshold and return the threshold that produces the best F1 score.
    """
    best_threshold = None
    best_f1 = 0.0
    # Create the validation dataloader once
    val_dataset = CSVValidationDataset(
        params['validation_csv_path'], 
        seq_len=params['seq_len'], 
        stride=params['stride']
    ...
    for thr in candidate_thresholds:
        ...
        logging.info(f"Evaluating candidate threshold: {thr:.4f}")
        precision, recall, f1 = validate_csv(model, params, device)
        logging.info(f"Threshold {thr:.4f} produced F1-Score: {f1:.4f}")
        if f1 > best_f1:
            best_f1 = f1
            best_threshold = thr
...
# -------------------------- Experiment Runner --------------------------
def run_experiment(global_params, device):
    """
    Run training with fixed hyperparameters and then search for the best threshold value.
    Returns a dictionary with the best F1 score, the best threshold, and training details.
    """
    # Use the global parameters as the fixed hyperparameters
    ...
    logging.info("Starting experiment with fixed hyperparameters:")
    ...
    for epoch in range(1, params['num_epochs'] + 1):
        ...
        for i, batch in enumerate(progress_bar):
            # For speed during training, limit the number of batches if desired.
            if i >= 1000:
                break
            features = batch['features'].to(device)
            optimizer.zero_grad()
    ...
    # After training, compute an initial threshold using update_threshold on the validation set.
    val_dataset = CSVValidationDataset(
        params['validation_csv_path'], 
        seq_len=params['seq_len'], 
        stride=params['stride']
    ...
    initial_threshold = update_threshold(model, val_loader, params, device, k=2.0)
    logging.info(f"Initial threshold from update_threshold: {initial_threshold:.4f}")

    # Define candidate thresholds starting from 10 units before the initial threshold up to 10 units above.
    candidate_thresholds = np.arange(initial_threshold - 0.5, initial_threshold + 0.55, 0.05)
    best_threshold, best_f1 = search_best_threshold(model, params, device, candidate_thresholds)
    params['threshold'] = best_threshold  # update the final threshold
...
# -------------------------- Main --------------------------
def main():
    # Global parameters with fixed hyperparameters.
    global_params = {
        ...
        'batch_size': 64,
        'num_epochs': 1,   # Use as many epochs as needed
        'patience': 3,
        # Fixed model hyperparameters:
        'dim_feedforward': 256,  # Will be updated as 4 * d_model
        'seq_len': 12,
        'stride': 12,  # This will be updated based on "stride_type".
        'alpha': 1.1,
        'beta': 5.0,
        'threshold': 5,  # initial threshold (will be tuned)
        'nhead': 8,
        'dropout': 0.5,
        ...
        'd_feature': 8,
        'num_layers': 1,
        'stride_type': "equal",  # Options: "equal", "half", "quarter"
    }
    ...
    result = run_experiment(global_params, device)
    logging.info("Experiment completed.")
    logging.info(f"Best F1-Score: {result['best_f1']:.4f}")
    logging.info(f"Best threshold value: {result['best_threshold']:.4f}")
    logging.info("Final hyperparameters:")
