...
###################################
# 2. MODEL DEFINITION (AUTOENCODER WITHOUT ATTENTION)
###################################
...
class CategoricalAutoencoder(nn.Module):
    def __init__(self, vocab_sizes, embedding_dims, hidden_dim, num_layers, dropout=0.1):
        ...
        embedding_dims: list of ints, e.g. [10, 6, 4, 5, 2, 4, 2]
        ...
        # Instead of attention, we now use the final hidden state of the LSTM as the context.
        # Two fully-connected layers to go from hidden -> 2*hidden -> hidden (latent)
        self.fc1 = nn.Linear(hidden_dim, 2 * hidden_dim)
        self.fc2 = nn.Linear(2 * hidden_dim, hidden_dim)
        # Decoder LSTM: decodes from the latent representation.
        self.decoder = nn.LSTM(
            input_size=hidden_dim, hidden_size=hidden_dim, num_layers=num_layers,
            batch_first=True, dropout=dropout
        )
        # Output heads: one per feature (to predict categorical distribution).
        self.output_heads = nn.ModuleList([nn.Linear(hidden_dim, vocab_size) for vocab_size in vocab_sizes])

    def encode(self, x):
        """
        Encodes the input x (shape [batch, seq_len, num_features]) into a latent vector.
        Uses an LSTM encoder and two FC layers.
        Returns:
          - latent: a [batch, hidden_dim] unit–normalized latent representation.
          - encoder_out: the raw LSTM outputs (unused in decoding here).
        """
        ...
        x_emb = torch.cat(embedded_feats, dim=-1)  # [batch, seq_len, total_emb_dim]
        # Get the encoder outputs and hidden states.
        encoder_out, (h_n, c_n) = self.encoder(x_emb)
        # Use the final hidden state of the last LSTM layer as the context.
        context = h_n[-1]  # shape: [batch, hidden_dim]
        # Pass through two FC layers with ReLU.
        latent = F.relu(self.fc1(context))
        latent = self.fc2(latent)
        # Normalize to unit hypersphere.
        latent = F.normalize(latent, p=2, dim=1)
        return latent, encoder_out

    def forward(self, x, return_latents=False):
        """
        x: tensor of shape [batch, seq_len, num_features]
        If return_latents is True, returns (outputs, latent).
        Otherwise, returns outputs (a list, one per feature, with shape [batch, seq_len, vocab_size]).
        For decoding, we simply repeat the latent vector for each time step.
        """
...
def compute_anomaly_scores(model, batch, vocab_sizes, device):
    """
    ...
    outputs = model(features)  # List of outputs (one per feature)
...
def produce_anomalies(model, val_loader, params, device, vocab_sizes):
    ...
    with torch.no_grad():
        with tqdm(total=len(val_loader), desc=f"Validation", unit="batch") as val_bar:
            for batch in val_loader:
                recon_scores = compute_anomaly_scores(model, batch, vocab_sizes, device)
                scores = recon_scores
    ...
    if len(all_scores) == 0:
        ...
        threshold = np.percentile(all_scores, 96)
...
###################################
# 4. TRAINING FUNCTION (AUTOENCODER ONLY)
###################################
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        with tqdm(total=len(train_loader), desc=f"Epoch {epoch}/{num_epochs}", unit="batch") as pbar:
            for batch in train_loader:
                ...
                batch_loss = 0.0
                # Compute loss for each feature without masking.
                for i in range(len(vocab_sizes)):
                    logits = outputs[i]
                    targets = features[:, :, i]
